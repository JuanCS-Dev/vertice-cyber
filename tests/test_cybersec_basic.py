    ...
    with patch("socket.gethostbyname", return_value="127.0.0.1"):
        ...
        with patch.object(agent, "_scan_ports", new_callable=AsyncMock) as mock_scan:
            ...
            with patch("httpx.AsyncClient") as mock_client:
                ...
                result = await agent.run_recon(
                    "example.com", scan_ports=False, scan_web=True
                )

                # Check if security issues were found
                assert any(
                    "Missing security headers" in issue
                    for issue in result.security_issues
                )
                assert any(
                    "Server header exposed" in issue for issue in result.security_issues
                )
