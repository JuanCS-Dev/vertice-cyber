...
GOVERNANCE_TOOLS = [
    ToolTestCase(
        ...
        arguments={"action": "Scan public network for vulnerabilities", "context": {"target": "192.168.1.0/24"}},
...
OFFENSIVE_TOOLS = [
    ...
    ToolTestCase(
        ...
        arguments={"scenario_id": "ransomware-attack", "target": "local"},  # Use valid scenario
...
ALL_TOOL_TESTS = GOVERNANCE_TOOLS + OSINT_TOOLS + THREAT_TOOLS + COMPLIANCE_TOOLS + OFFENSIVE_TOOLS + AI_TOOLS
...
class TestAllMCPTools:
    ...
    def test_tool_execution(self, client: TestClient, test_case: ToolTestCase, execution_metrics):
        ...
        if test_case.category not in execution_metrics["by_category"]:
            execution_metrics["by_category"][test_case.category] = {"count": 0, "time_ms": 0}
        execution_metrics["by_category"][test_case.category]["count"] += 1
        execution_metrics["by_category"][test_case.category]["time_ms"] += execution_time_ms
        
        # Assertions
        assert response.status_code == 200, f"Tool {test_case.name} returned {response.status_code}"
                ...
                if test_case.expected_fields and data.get("result"):
                    ...
                    if isinstance(result, dict):
                        for field in test_case.expected_fields:
                            assert field in result, f"Missing field '{field}' in {test_case.name} response"
            else:
...
class TestToolCategories:
    ...
    def test_governance_tools_count(self, client: TestClient):
        ...
        assert len(governance_tools) >= 4, f"Expected >=4 governance tools, got {len(governance_tools)}"
    ...
    def test_intelligence_tools_count(self, client: TestClient):
        ...
        assert len(intel_tools) >= 6, f"Expected >=6 intel tools, got {len(intel_tools)}"
    ...
    def test_offensive_tools_count(self, client: TestClient):
        ...
        assert len(offensive_tools) >= 4, f"Expected >=4 offensive tools, got {len(offensive_tools)}"
