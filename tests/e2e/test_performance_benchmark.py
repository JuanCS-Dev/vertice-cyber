...
class TestAPILatency:
    ...
    def test_health_check_latency(self, client: TestClient):
        ...
        assert result.avg_time_ms < 50, (
            f"Health check too slow: {result.avg_time_ms:.1f}ms"
        )
...
class TestConcurrency:
    ...
    def test_concurrent_health_checks(self, client: TestClient):
        ...
        assert total_elapsed < 5000, "Too slow under concurrency"
    ...
    def test_concurrent_tool_execution(self, client: TestClient):
        ...
        def execute_tool(tool_name: str, args: dict):
            start = time.perf_counter()
            response = client.post(
                "/mcp/tools/execute", json={"tool_name": tool_name, "arguments": args}
            )
            elapsed = (time.perf_counter() - start) * 1000
        ...
        assert success_count >= num_requests - 1, "Too many failures"
